...
function morningPaletteV10_(kind) {
  const p = {
    HOT:      {bg:'#FDE2E2', fg:'#8F2D2D'},
    COLD:     {bg:'#E6F0FA', fg:'#245A7A'},
    NEUTRAL:  {bg:'#E5F5EA', fg:'#1F6B3A'},
    WARN:     {bg:'#FFF3CD', fg:'#7A5B00'},
    FOMO:     {bg:'#FFE9D6', fg:'#9A4D00'},
    MACD:     {bg:'#EDF1F5', fg:'#44546A'},
    INFO:     {bg:'#EAF4FB', fg:'#245A7A'},
...
function polishMorningRadarV10_(sheet, assets) {
  ...
  // RADAR 13 ALTÓW — TERAZ CAŁY WIERSZ dziedziczy kolor statusu RADAR.
  // Potem pojedyncze alarmowe komórki RSI/MFI/MACD/FOMO mogą nadpisać tło.
  for (let i=0; i<n; i++) {
    const r = 20 + i;
    ...
    // MACD: alert -> szaro-niebieski jak MACD ALERT.
...
print('V10 Morning Radar full-row color logic patched')
