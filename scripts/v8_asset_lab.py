...
def etf_context(inputs,asset):
    p=asset.lower()
    one=sf(inputs.get(f'{p}_etf_1d_usdm')); five=sf(inputs.get(f'{p}_etf_5d_usdm')); twenty=sf(inputs.get(f'{p}_etf_20d_usdm')); thirty=sf(inputs.get(f'{p}_etf_30d_usdm')); accel=sf(inputs.get(f'{p}_etf_accel_5d_usdm'))
    ...
    name='BTC' if asset=='BTC' else 'ETH'
    return {'status':'OK','label_pl':label,'flow_1d_usdm':one,'flow_5d_usdm':five,'flow_20d_usdm':twenty,'flow_30d_usdm':thirty,'acceleration_5d_usdm':accel,'score_adjustment':adj,'explanation_pl':f'Dodatnia wartość oznacza napływ kapitału do amerykańskich spot ETF {name}; ujemna oznacza odpływ.'}
...
def fast_score(t1,t4,td,typ,oc,phase,mm,sym):
    ...
    if typ in ('CRYPTO','MEME'):
        ...
        if sym in ('BTC','ETH'):
            etf=etf_context(phase.get('inputs') or {},sym)
            etf_adj=sf(etf.get('score_adjustment')) or 0.0; score+=etf_adj
...
def main():
    ...
    status={'generated_at_utc':NOW.isoformat(),'engine':'V8_ASSET_LAB_v0.5','active_assets':len(reports),'fast_mode_ready':True,'fast_multitimeframe_enabled':True,'deep_mode_ready':True,'deep_comparison_enabled':True,'polish_user_language':True,'btc_eth_dedicated_etf_filter':True,'crypto_checkpoints':['D0','D1','D3','D7','D30'],'stock_checkpoints':['D0','D7','D30'],'portfolio_connection':False,'execution_connection':False}
