...
lab_ready = (
    ...
    and lab_status.get('fast_mode_ready') is True
    and lab_status.get('deep_mode_ready') is True
    and lab_status.get('polish_user_language') is True
    and lab_status.get('btc_eth_dedicated_etf_filter') is True
    and exists('lab/LAB_CHECKPOINTS.csv')
    and exists('lab/LAB_CHECKPOINT_STATUS.json')
    and exists('lab/LAB_DEEP_REPORT.md')
    and exists('lab/LAB_HISTORY.csv')
)
...
# 10 głównych obszarów = 95 pkt, rozszerzony AAVE/HBAR = 5 pkt
areas = [
    ...
    ('LAB + PUNKTY KONTROLNE', 10, lab_ready, 'Szybka analiza, głęboka obserwacja, historia i trwałe punkty kontrolne.'),
...
core_ready = all(ok for _, _, ok, _ in areas)

payload = {
    'generated_at_utc': NOW,
    'engine': 'V8_FINAL_READINESS_v1.1',
    'readiness_percent': score,
    'status_pl': status_pl,
    'core_production_ready': core_ready,
...
print(json.dumps({'readiness_percent': score, 'status_pl': status_pl, 'core_production_ready': core_ready, 'blockers': blockers}, ensure_ascii=False, indent=2))
